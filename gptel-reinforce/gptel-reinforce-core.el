...
(defconst gptel-reinforce-default-summarizer-prompt
  (string-join
   '("You maintain a running summary of user feedback about a general text artifact."
     ...
     "When evidence is narrow, keep the inferred scope narrow."
     ...
     "Preserve any explicit user preference about keeping an existing affordance unless later evidence clearly contradicts it."
...
(defconst gptel-reinforce-default-updater-prompt
  (string-join
   '("You revise a general text artifact from a feedback summary."
     ...
     "Preserve existing affordances unless the summary clearly says they should be removed."
     "When feedback targets a specific trigger, condition, command, rule, or context, prefer a local fix over a broad policy change."
