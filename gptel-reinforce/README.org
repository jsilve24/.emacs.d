...
The base commands (=gptel-reinforce-like=, =-dislike=, =-neutral=, =-score=) auto-detect context: if point is inside annotated LLM output they record output-feedback against that artifact; otherwise they record item-feedback against the current item. These are the ones you typically bind to keys — they do the right thing in both contexts.
...
*=gptel-reinforce-set-active-database DATABASE=* — pins a database to the current buffer. Feedback commands normally auto-detect which database applies by calling every registered context function and using whichever returns a non-nil result. This works fine when you are in an Elfeed buffer because that context function returns nil everywhere else. But if you register a custom database whose context function is broad (e.g., it returns something in any buffer, or in all Org files), auto-detection may pick the wrong database or prompt you to choose every time. Pinning a database with =set-active-database= bypasses auto-detection entirely for that buffer. The typical use is in a mode hook:
