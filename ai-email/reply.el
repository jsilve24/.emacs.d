...
are appended here."
  (let* ((today-time (current-time))
    ...
    (concat
     ...
     "A meeting is genuinely confirmed only when the visible thread shows explicit acceptance from everyone whose availability matters, or when you are sending the final needed acceptance to a previously proposed concrete slot.\n"
     "A proposal from one attendee is not a confirmation. If another attendee has not explicitly agreed yet, stay in coordination mode, use proposal language, and do not call stage_calendar_capture.\n"
