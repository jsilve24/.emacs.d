...
(defconst jds/snippets-reinforce-artifact "yasnippets"
  "Artifact name for the full yasnippet collection.")

(defconst jds/snippets-reinforce-dir
  (expand-file-name "snippets" user-emacs-directory)
  "Root directory of yasnippet files.")

(defun jds/snippets-reinforce--serialize ()
  "Return all snippets as a single structured string.
Each snippet is preceded by a # === MODE/NAME === header line."
  (let (parts)
    (dolist (file (sort (directory-files-recursively
                         jds/snippets-reinforce-dir "." t)
                        #'string<))
      (when (and (file-regular-p file)
                 (not (string-suffix-p ".md" file)))
        (let* ((rel (file-relative-name file jds/snippets-reinforce-dir))
...
(defun jds/snippets-reinforce--write-back (text)
  "Parse TEXT and write each snippet section back to its file, then reload."
  (let ((pos 0))
    (while (string-match "^# === \\([^\n]+\\) ===\n" text pos)
      (let* ((rel-path  (string-trim (match-string 1 text)))
             (body-start (match-end 0))
             (next-hdr   (string-match "^# === " text body-start))
             (body       (substring text body-start (or next-hdr (length text))))
             (file       (expand-file-name rel-path jds/snippets-reinforce-dir)))
  ...
  (when (fboundp 'yas-reload-all)
    (yas-reload-all)
    (message "Snippets updated and reloaded")))

;; Track expanded snippet regions so like/dislike works on them.
;; We advise yas-exit-snippet: grab the snippet bounds as markers BEFORE the
;; overlay is removed, then annotate AFTER the original function runs.
(defun jds/snippets-reinforce--exit-advice (orig-fn snippet &rest args)
  "Around-advice on `yas-exit-snippet' to annotate the expanded region."
  (let* ((s  (and (fboundp 'yas-snippet-start) (yas-snippet-start snippet)))
         (e  (and (fboundp 'yas-snippet-end)   (yas-snippet-end   snippet)))
         (sm (when (and s e (< s e)) (copy-marker s)))
         (em (when sm                (copy-marker e t))))
    (apply orig-fn snippet args)
    (when (and sm (marker-buffer sm)
               (fboundp 'gptel-reinforce-track-output-region)
               (< (marker-position sm) (marker-position em)))
      (gptel-reinforce-track-output-region
       jds/snippets-reinforce-artifact sm em))
    (when sm (set-marker sm nil))
    (when em (set-marker em nil))))

(with-eval-after-load 'yasnippet
  (advice-add 'yas-exit-snippet :around #'jds/snippets-reinforce--exit-advice))

(gptel-reinforce-register-database
 :name "yasnippets"
 :candidate-fn (lambda ()
                 (list :context
                       (list :item-key "yasnippets-collection"
                             :title "yasnippet collection"))))

(gptel-reinforce-register-artifact
 :name jds/snippets-reinforce-artifact
 :database "yasnippets"
 :type "code"
 :initial-text (jds/snippets-reinforce--serialize)
 :post-update-hook #'jds/snippets-reinforce--write-back)
